...
func TestSearchGoldenAtFixedDepthAndNodes(t *testing.T) {
	...
	for _, fixture := range []struct {
	...
	}{
		{
			...
			wantDepth: Result{Action: move(2, 3), Score: 5331, Depth: 2, Nodes: 216, Evaluations: 199},
			wantNodes: Result{Action: move(2, 3), Score: 5331, Depth: 2, Nodes: 1000, Evaluations: 916, BudgetExhausted: true},
		},
		{
			name: "maxn", state: three,
			wantDepth: Result{Action: move(1, 2), Score: 1246, Depth: 2, Nodes: 46, Evaluations: 40},
			wantNodes: Result{Action: move(1, 2), Score: 4321, Depth: 3, Nodes: 1000, Evaluations: 814, BudgetExhausted: true},
		},
	} {
		t.Run(fixture.name, func(t *testing.T) {
			depth, ok := ChooseDepth(context.Background(), fixture.state, 2)
			if !ok || depth != fixture.wantDepth {
				t.Fatalf("fixed-depth result = %+v ok=%v, want %+v", depth, ok, fixture.wantDepth)
			}
			nodes, ok := ChooseNodeBudget(fixture.state, 1000)
			if !ok || nodes != fixture.wantNodes {
				t.Fatalf("fixed-node result = %+v ok=%v, want %+v", nodes, ok, fixture.wantNodes)
			}
		})
	}
}
